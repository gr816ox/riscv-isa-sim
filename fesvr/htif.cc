...
std::map<std::string, uint64_t> htif_t::load_payload(const std::string& payload, reg_t* entry)
{
  ...
  {
    test_path = PREFIX TARGET_DIR + payload;
  ...
  if (path.empty()){
    if (payload.find('/') == std::string::npos)
      throw std::runtime_error(
        "could not open " + test_path +
        " which is the absolute path spike generated by /$prefix/$with-target/bin/<target program> " + 
        " (did you misspell it? If VCS, did you forget +permissive/+permissive-off?)");
    throw std::runtime_error(
        "could not open " + payload +
        " (did you correctly configure --prefix and --with-target when execute ../figure? " +
        "did you misspell it? If VCS, did you forget +permissive/+permissive-off?)");
  }
